...
from depth_dataloader import DatasetDepth
...
def get_args():
    ...
    parser.add_argument('--output_dir', default='./output_dir/depth')
...
def calculate_metric(args, target, ours):
    ...
    target = np.clip(target * 10000, 0, 10000)
    ours = np.clip(ours * 10000, 0, 10000)
    target = target.mean(0).astype(np.int16)
    ours = ours.mean(0).astype(np.int16)

    target = target.astype(np.float32) / 1000.0
    ours = ours.astype(np.float32) / 1000.0
    ours[ours < 0.001] = 0.001
    ours[ours > 10] = 10
    ours[np.isinf(ours)] = 10
    target[np.isinf(target)] = 0
    target[np.isnan(target)] = 0

    valid_mask = np.logical_and(target > 0.001, target < 10)

    # a = target[valid_mask] - ours[valid_mask]
    # b = np.isnan(a)
...
def evaluate(args):
    ...
    ds = DatasetDepth(args.data_path, image_transform, mask_transform)
