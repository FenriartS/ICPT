...
class DatasetDeraining(Dataset):
    def __init__(self, datapath, image_transform, mask_transform, padding: bool = 1,
        ...
        self.trn = trn
        if trn:
            self.img_src_dir = "../datasets/derain/train/"
        else:
            self.img_src_dir = "../datasets/derain/test/Test1200/"
        self.img_path_list = glob.glob(self.img_src_dir + "input/" + "*.png") + glob.glob(self.img_src_dir + "input/" + "*.jpg")
    ...
    def __getitem__(self, idx):
        ...
        query_trg = query_img.replace('input', 'target')
        support_trg = support_img.replace('input', 'target')
