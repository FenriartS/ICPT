...
def get_args():
    ...
    parser.add_argument('--output_dir', default='./output_dir/foreground_segm/split_all')
...
def _generate_result_for_canvas(args, model, canvas):
    ...
    # im_paste = im_paste[0]
    # im_paste = im_paste.cpu().detach().int()
    canvas = canvas[0]
    canvas = torch.einsum('chw->hwc', canvas)
    canvas = torch.clip((canvas.cpu().detach() * imagenet_std + imagenet_mean) * 255, 0, 255).int().numpy()
...
def evaluate(args):
    with open(os.path.join(args.output_dir, 'log_in.txt'), 'w') as log:
        log.write(str(args) + '\n')
    padding = 1
    ...
    mask_transform = torchvision.transforms.Compose(
        [torchvision.transforms.Resize((224 // 2 - padding, 224 // 2 - padding), 3),
         torchvision.transforms.ToTensor()])
    # image_transform = torchvision.transforms.Compose(
    #     [torchvision.transforms.Resize((64 - padding, 64 - padding), 3),
    #      torchvision.transforms.ToTensor()])
    # mask_transform = torchvision.transforms.Compose(
    #     [torchvision.transforms.Resize((64 - padding, 64 - padding), 3),
    #      torchvision.transforms.ToTensor()])
    ...
    for idx in trange(len(ds)):
        ...
        # canvas = torch.stack((canvas, ds[idx]), dim=0).to(args.device)
        ...
        # original_image_coarse, generated_result_coarse = _generate_result_for_canvas(args, model, canvas_coarse)
        # generated_result = generated_result_coarse
        if args.output_dir:
        ...
        with open(os.path.join(args.output_dir, 'log_in.txt'), 'a') as log:
