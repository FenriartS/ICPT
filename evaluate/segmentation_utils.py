...
def calculate_metric(args, target, ours, fg_color=WHITE, bg_color=BLACK):
    ...
    ours = ours[113:, 113:]
    # target = target[80:143, 80:143]
    # ours = ours[80:143, 80:143]
...
def _calc_metric(ours, target, fg_color=WHITE, bg_color=BLACK):
    ...
    a = fg_color[np.newaxis, np.newaxis, :]
    b = ((ours - fg_color[np.newaxis, np.newaxis, :]) != 0).any(axis=2)
    c = ((ours - np.array(bg_color)[np.newaxis, np.newaxis, :]) != 0).any(axis=2)
    d = b & c
    numcolor = sum(sum(d))
    # color = ((ours - fg_color[np.newaxis, np.newaxis, :]) != 0 and ((ours - bg_color[np.newaxis, np.newaxis, :]) != 0)).all(axis=2)
